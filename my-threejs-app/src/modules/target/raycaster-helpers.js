...
async function buildSelFromRayHit(engineComponents, rayHit) {             // builds selection info from a ray hit
  ...
  let sel = { modelId, itemId };                                          // minimal selection payload(data package) (fallback(backup/default)) to keep working when fragments API is unavailable, when there is no more data coming
  try {
