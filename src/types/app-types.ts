...
export type JsonValueType =
  ...
  | JsonValueType[];
export type JsonObjectType = { [key: string]: JsonValueType | undefined };
...
export type MarkerServiceType = {
  create(world: World, markerLabelElemTemp: HTMLElement, markerWorldPosition: Vector3, isStatic: boolean): string | null;
...
export type IfcLoaderLikeType = {
  ...
  load(bytes: Uint8Array, coordinate: boolean, modelId: string): Promise<object>;
};

// Small viewer-world shape used by app modules.
export type ViewerWorldType = World;

export type FragmentModelType = {
  object: Object3D;
  useCamera(camera: PerspectiveCamera | OrthographicCamera | Camera): void;
};
